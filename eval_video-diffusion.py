...
def main(args):
    ...
    wandb_run = wandb.init(
        project='iclr_ssm_diffusion_evalation', 
        entity="shim0114", 
        config=vars(args))
    ...
    elif args.dataset == 'minerl' or args.dataset =='gqn' or args.dataset == 'carla':
    ...
    #     num_reals_sample = 1
    count = 0
    # for _ in range(num_reals_sample):
    for reals_video in tqdm(dl):
        ...
        if args.dataset == 'ucf101-all' or args.dataset =='gqn':
            if count >= 2000:
                break
    ...
    for fakes_path in tqdm(all_fakes_path):
        ...
        if int(sample_seed) in args.sample_seeds:
            if not int(number_a) > args.num_samples:
                ...
                if fakes_video.shape[0] % args.eval_batch_size != 0:
                    raise ValueError('batch size should be divisible by number of samples')
                else:
                    for i in range(fakes_video.shape[0] // args.eval_batch_size):
                        tmp_fakes_video = fakes_video[i * args.eval_batch_size: (i + 1) * args.eval_batch_size]
                        fakes_activation = compute_acts(tmp_fakes_video)
